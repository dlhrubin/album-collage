...
class App extends Component {
  constructor(props) {
    ...
    handleResize = () => {
      ...
      const windowWidth = Math.max(document.body.scrollWidth, document.documentElement.scrollWidth, document.body.offsetWidth, document.documentElement.offsetWidth, document.documentElement.clientWidth);
    ...
    handleShuffle = () => {
      ...
      while (sameAsBefore) {
        ...
        for (let i = 0; i < this.state.selections.length; i++) {
          ...
          sameAsBefore = (sameAsBefore === false) ? false : (shuffled[i] === this.state.selections[i]);
        }
      }
      this.setState({
        selections: shuffled,
    ...
    render() {
      ...
      const panelToDisplay = (!this.state.hidePanel) ? '' : (this.state.selections.length && !this.state.editing) ? 'collage' : 'menu';
