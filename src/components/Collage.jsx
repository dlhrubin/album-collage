...
export class Collage extends Component {
  ...
  componentDidUpdate(prevProps) {
    if ((this.props.shape !== prevProps.shape) || (this.props.selections.length !== prevProps.selections.length)) {
      const newWidth = this.props.shape ? collageDimensions[`${this.props.shape}-${this.props.selections.length}`][0] * 174 : 0;
      const newHeight = this.props.shape ? collageDimensions[`${this.props.shape}-${this.props.selections.length}`][1] * 174 : 0;
      this.setState({
        width: newWidth,
        height: newHeight,
      });
    ...
    render() {
      ...
      const buttonStyle = { opacity: (this.props.editing || !this.props.shape) ? '0.3' : '' };
      const buttonDisabled = !!((this.props.editing || !this.props.shape));
      // On small screens, hide collage if collage is being created or edited in the selection menu
      const collageDisplay = (this.props.panelToDisplay === 'menu') ? 'none' : '';

      return (
        <section className="collage" style={{ display: collageDisplay, backgroundColor: this.props.test }}>
          <div className="edit-dock">
            <button ref={this.editButton} aria-label="Edit Collage" onClick={this.props.editCollage} style={{ backgroundColor: this.props.editing ? 'var(--highlight)' : '', borderColor: this.props.editing ? 'var(--highlight)' : '', opacity: !this.props.shape ? '0.3' : '' }} disabled={!this.props.shape}>
