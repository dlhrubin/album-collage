...
class AlbumSelect extends Component {
  ...
  componentDidMount() {
    ...
    handleSearch = (isArtist, e) => {
      ...
      const hasContent = (isArtist) ? this.state.newArtist : this.state.newAlbum;
      if (hasContent) {
        if (isArtist) {
          ...
          axios.get(`https://ws.audioscrobbler.com/2.0/?method=album.search&api_key=${this.state.key}&album=${this.state.newAlbum}&format=json`)
            .then((res) => {
              ...
              const albumsFound = res.data.results.albummatches.album.filter((album) => album.artist === this.state.newArtist);
              ...
              if (albumsFound.length && albumsFound[0].image[0]['#text']) {
                ...
                const notRepeat = (this.props.selections.filter((selection) => (selection.artist === this.state.newArtist && selection.album === albumsFound[0].name)).length === 0);
                if (notRepeat) {
                  this.setState({
    ...
    render() {
      ...
      const selectedAlbums = this.props.selections.map((selection, i) => (
      ...
      const inputStyle = (field) => ({ borderColor: this.state.warnings[field] ? 'red' : '', width: this.props.inputWidth });
      // Add red border to search boxes when a warning is present
      const warningBorder = (field) => (this.state.warnings[field] ? { borderColor: 'red' } : { borderColor: '' });
        ...
        <div className="album-options">
          ...
          <form onSubmit={this.handleSearch.bind(this, true)}>
            <div onClick={this.handleClick}>
              ...
              <input id="artist-search" type="text" spellCheck="false" style={inputStyle('artist')} placeholder="Enter artist name..." disabled={(this.props.selections.length === this.props.albumRange.max) ? 'disabled' : ''} ref={this.artistInput} value={this.state.newArtist} onChange={this.handleChange.bind(this, true)} />
          ...
          <form onSubmit={this.handleSearch.bind(this, false)} style={{ visibility: this.state.searchVis }}>
            <div>
              ...
              <input id="album-search" type="text" spellCheck="false" style={inputStyle('album')} placeholder="Enter album name..." ref={this.albumInput} value={this.state.newAlbum} onChange={this.handleChange.bind(this, false)} />
          ...
          <div className="album-selection">
            <p>
              Selection (
              {this.props.albumRange.min}
              -
              {this.props.albumRange.max}
              {' '}
              albums)
            </p>
